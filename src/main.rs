...
fn lift_file(path: &str) -> i32 {
    ...
    // Three populations, kept apart. Only the first is a reading of functions:
    // descent followed the program's own edges to get there. The other two are
    // linear scans of what descent never reached, cut wherever the scan stopped,
    // so an F in them is a truth about the cut and not about the code. Pooling
    // them made a swept fragment report as an ill-typed function.
    let mut tally = [0usize; 4];     // descended functions
    let mut sweptly = [0usize; 4];   // swept runs, both sweeps
    let mut illtyped: Vec<(u64, String)> = Vec::new();
    let mut claimed: std::collections::BTreeSet<u64> = std::collections::BTreeSet::new();
    for (n, (start, f)) in w.functions.iter().enumerate() {
        for ins in f { claimed.insert(ins.address); }
        let word = vox::recompile_function(f);
        let t = if n < w.descended { &mut tally } else { &mut sweptly };
        match vox::verdict(&word) {
            'T' => t[0] += 1, 'B' => t[1] += 1, 'N' => t[2] += 1,
            _ => {
                t[3] += 1;
                if n < w.descended && illtyped.len() < 8 {
                    illtyped.push((*start, vox::glyphs(&word)));
                }
            }
        }
    }
    let descended = w.descended;
    let in_walk_swept = w.functions.len() - w.descended;
    ...
    for (base, bytes) in &image.segments {
        ...
        while pos < bytes.len() {
            let addr = base + pos as u64;
            if claimed.contains(&addr) { flush(&mut cur, &mut sweptly, &mut swept); 
            ...
            match x86::decode(&bytes[pos..], addr) {
                Some(d) if d.len > 0 => {
                    ...
                    cur.push(d);
                    if terminal { flush(&mut cur, &mut sweptly, &mut swept); }
                }
                _ => { flush(&mut cur, &mut sweptly, &mut swept); pos += 1; }
            }
        }
        flush(&mut cur, &mut sweptly, &mut swept);
    }
    let total_cov = w.claimed_bytes + swept_bytes;
    println!();
    let swept = swept + in_walk_swept;
    if swept > 0 {
    ...
    println!("  verdicts  T {}   B {}   N {}   F {}", tally[0], tally[1], tally[2], tally[3]);
    if swept > 0 {
        println!("  swept     T {}   B {}   N {}   F {}   (fragments, not functions)",
            sweptly[0], sweptly[1], sweptly[2], sweptly[3]);
    }
    for (a, g) in &illtyped {
