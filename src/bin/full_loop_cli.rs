...
fn main() {
    ...
    println!("=== full loop: v2(unmatched->N) -> MEASURE {} -> SCAN-APPEND {} ===", OP_MEASURE, OP_SCAN_APPEND);
    ...
    for gen in 0..6usize {
        ...
        let r_imasm = RouterObject::decode(&st.router_word()).expect("decode");
        let eq = r_host.word == r_imasm.word;
        total += 1; if eq { agree += 1; }
        println!("gen {}: v2 {}  clauses {} -> host {} / imasm {}  eq {}", gen, v.name(), rk.transitions.len(), r_host.transitions.len(), r_imasm.transitions.len(), eq);
        rk = r_imasm;
        if word == OP_PRESERVE { break; }
    }
    println!("verdict sequence: {:?}", verdicts);
    println!("AGREEMENT: {}/{}   FULL-LOOP: {}", agree, total, if agree == total { "PASS" } else { "OPEN" });
}
