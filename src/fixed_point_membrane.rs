...
fn collapse_nested_tower(
    ...
    // Repeated residues overwrite older labels so the fuse retains the nearest
    // inner representative to the outer branch.
    let mut inner: BTreeMap<Tape, Tape> = BTreeMap::new();
    let mut j = zero_t;
    while cmp(&j, &m) == Ordering::Less {
        let phase = spectral.modular_branch(&j).map_err(String::from)?;
        inner.insert(phase, j.clone());
        j = add(&j, &one_t);
    }
    ...
    while cmp(&i, &m) != Ordering::Greater {
        ...
        if let Some(inner_exponent) = inner.get(&phase) {
