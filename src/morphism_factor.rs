...
/// Brent's variant of Pollard's rho over arbitrary-width tapes.
///
/// A genuine general-purpose factor engine on the Grammar's numeral tapes: the
/// orbit `y ← y²+c (mod n)` is a winding on the finite residue ring, and a
/// non-trivial `gcd(∏|x−y|, n)` is the address where two winding branches fold
/// onto one residue — the fold that names a factor. Returns a proper divisor of
/// `n` (`1 < d < n`) or `None` if the step budget is spent without a fold.
pub fn pollard_rho_brent(n: &[char], budget: u64) -> Option<Tape> {
    let one_t = one();
    let two_t = tape_u64(2);
    if cmp(n, &two_t) != core::cmp::Ordering::Greater { return None; }
    // even n: 2 is the fold immediately.
    if n.last().map(|c| "02468".contains(*c)).unwrap_or(false) {
        return Some(two_t);
    }
    let modmul = |x: &[char], y: &[char]| -> Tape { modulo(&mul(x, y), n) };
    let abs_diff = |x: &[char], y: &[char]| -> Tape {
        match cmp(x, y) {
            core::cmp::Ordering::Less => sub(y, x),
            _ => sub(x, y),
        }
    };
    let mut c = one_t.clone();
    let mut steps: u64 = 0;
    // Retry with a fresh additive constant c when a run collapses to g==n.
    while cmp(&c, &tape_u64(20)) == core::cmp::Ordering::Less {
        let mut y = two_t.clone();
        let mut g = one_t.clone();
        let mut r: u64 = 1;
        let m: u64 = 128;
        let mut x = y.clone();
        let mut ys = y.clone();
        while cmp(&g, &one_t) == core::cmp::Ordering::Equal {
            x = y.clone();
            for _ in 0..r {
                y = modulo(&add(&mul(&y, &y), &c), n);
            }
            let mut k: u64 = 0;
            while k < r && cmp(&g, &one_t) == core::cmp::Ordering::Equal {
                ys = y.clone();
                let lim = core::cmp::min(m, r - k);
                let mut q = one_t.clone();
                for _ in 0..lim {
                    y = modulo(&add(&mul(&y, &y), &c), n);
                    q = modmul(&q, &abs_diff(&x, &y));
                    steps += 1;
        ...
        if cmp(&g, n) == core::cmp::Ordering::Equal {
            // backtrack single steps from ys to isolate the fold
            loop {
                ys = modulo(&add(&mul(&ys, &ys), &c), n);
                g = gcd(abs_diff(&x, &ys), n.to_vec());
                steps += 1;
                if cmp(&g, &one_t) != core::cmp::Ordering::Equal { break; }
                if steps >= budget { return None; }
            }
        }
