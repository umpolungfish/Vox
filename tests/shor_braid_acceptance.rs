use vox::morphism_factor::{decimal_to_tape, dec_of, mul, cmp};
use vox::shor_braid::{shor_braid, shor_factor_via_braid};
use vox::winding_readout::{winding_number, winding_number_tape};
...
fn check(a: &str, n: &str, r_exp: &str) {
    ...
    assert!(w.len() <= 8 + 6 * lv + rval.min(64), "word O(log N)+comb: len={} levels={}", w.len(), lv);
    ...
    println!("OK N={n} a={a} r={r} levels={lv} wordlen={} factors={} x {}", w.len(), dec_of(&p), dec_of(&q));
}
