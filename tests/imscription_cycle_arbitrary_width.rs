...
fn baked_forty_one_digit_semiprime_closes_as_a_native_tape_cycle() {
    ...
    println!(
        "imscription arbitrary width: baked N digits={} tape_marks={} descents=1 support=39 fixed=true",
        dec_of(&n).len(),
        n.len(),
    );
}
...
fn arbitrary_width_bulk_changes_lattice_without_narrowing_the_boundary() {
    ...
    println!(
        "imscription arbitrary-width lattice change: p={} q={} N_digits={} descents={} k=2 support=63 fixed=true",
        dec_of(&p),
        dec_of(&q),
        dec_of(&n).len(),
        summary.descents,
...
fn lehman_boundary_reimscribes_past_the_old_sixty_four_multiplier_wall() {
    ...
    println!(
        "imscription unbounded multiplier: k=96 descents=98 quotient_transforms=98 old_k64_wall_crossed=true fixed=true marks={}",
        wire.len(),
    );
}
